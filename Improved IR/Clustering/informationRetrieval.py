...
class InformationRetrieval():
	...
	def buildIndex(self, docs, docIDs):
		...
		self.kmeans.fit(self.index)
		self.cluster_dict = {i: [] for i in range(k)}
		for i,label in enumerate(self.kmeans.labels_):
			self.cluster_dict[label].append(i)
	...
	def rank(self, queries):
		...
		for query in query_index:
			...
			center = self.kmeans.predict(query.reshape(1,-1))
			center = center[0]
			sims = cosine_similarity(query.reshape(1,-1),self.index[self.cluster_dict[center]])[0]
			top_ids = [i for sim,i in sorted(list(zip(sims,self.cluster_dict[center])),key=lambda x:x[0],reverse=True)]
